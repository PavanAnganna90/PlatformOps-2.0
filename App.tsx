...
import { Bell, Search, Settings, ChevronDown, Loader2 } from 'lucide-react';
...
const MainLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] font-sans text-slate-100 flex overflow-hidden">
      ...
      <main className="flex-1 ml-72 h-screen flex flex-col relative z-10">
        ...
        <header className="h-24 flex justify-between items-center px-8 shrink-0">
          ...
          <div className="relative group w-96">
            <div className="absolute inset-0 bg-gradient-to-r from-primary/20 to-accent/20 rounded-full blur opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <div className="relative bg-[#0F1115] border border-white/5 rounded-full flex items-center px-4 py-2.5 shadow-lg">
               <Search className="w-4 h-4 text-slate-500 mr-3" />
               <input 
                 type="text" 
                 placeholder="Search assets, logs, or traces..." 
                 className="bg-transparent border-none focus:outline-none text-sm text-white w-full placeholder:text-slate-600"
               />
               <div className="px-2 py-0.5 rounded-md bg-white/5 border border-white/5 text-[10px] text-slate-500 font-mono">⌘K</div>
            </div>
          </div>

          <div className="flex items-center gap-6">
            <div className="text-[10px] font-mono border border-slate-800 px-2 py-1 rounded text-slate-500">
            ...
            <div className="pl-6 border-l border-white/5">
              <button className="flex items-center gap-3 p-1 pr-4 rounded-full bg-[#0F1115] border border-white/5 hover:border-white/10 transition-colors">
