...
import { ArrowUpRight, ArrowDownRight, Activity, Terminal, Zap, Bug, Skull, WifiOff, Play, Pause, MoreHorizontal, Wallet, TrendingUp } from 'lucide-react';
...
export const DashboardView: React.FC = () => {
  ...
  const activeIncidents = nodes.filter(n => n.status === Status.WARNING || n.status === Status.ERROR).length;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-3 bg-[#0F1115] p-1.5 rounded-2xl border border-white/5 shadow-lg">
           <button 
             onClick={toggleSimulation}
             className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
               isSimRunning ? 'bg-white/5 text-slate-300' : 'bg-primary text-white shadow-[0_0_15px_-3px_rgba(99,102,241,0.5)]'
             }`}
           >
             {isSimRunning ? 'Pause Sim' : 'Start Sim'}
           </button>
           <div className="flex gap-1 pr-2">
              <button onClick={() => handleChaos('cpu_spike')} className="p-2 hover:bg-white/5 rounded-lg text-slate-400 hover:text-white transition-colors" title="Inject CPU Spike"><Activity size={18} /></button>
              <button onClick={() => handleChaos('pod_crash')} className="p-2 hover:bg-white/5 rounded-lg text-slate-400 hover:text-white transition-colors" title="Crash Pod"><Skull size={18} /></button>
           </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4 px-1">
          <h3 className="text-lg font-semibold text-slate-200">Key Performance Indicators</h3>
          <div className="flex gap-2">
             <span className="text-xs px-3 py-1 rounded-full bg-white/5 border border-white/5 text-slate-400">24H</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="h-[450px] flex flex-col" hoverEffect>
            <div className="flex justify-between items-start mb-8">
              <div>
                 <h3 className="text-xl font-bold text-white mb-1">Cluster Performance</h3>
                 <p className="text-sm text-slate-400">Real-time resource utilization across nodes</p>
              </div>
              <div className="flex gap-4">
                 <div className="flex flex-col items-end">
                    <span className="text-xs text-slate-500 uppercase tracking-wider">Avg CPU</span>
                    <span className="text-lg font-mono text-white">42.5%</span>
                 </div>
                 <div className="flex flex-col items-end">
                    <span className="text-xs text-slate-500 uppercase tracking-wider">Avg Mem</span>
                    <span className="text-lg font-mono text-white">68.2%</span>
                 </div>
              </div>
            </div>
            ...
            <div className="flex-1 w-full min-h-0 -ml-2">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={mainChartData}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#0F1115', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', fontSize: '12px', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.5)' }}
        ...
        <Card className="flex flex-col h-[450px]">
          <div className="flex justify-between items-center mb-6">
             <h3 className="font-bold text-white">Active Nodes</h3>
             <button className="p-2 hover:bg-white/5 rounded-lg text-slate-400"><MoreHorizontal size={18} /></button>
          </div>

          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {nodes.map(node => (
               <div key={node.id} className="group p-3 rounded-2xl bg-[#0F1115] border border-white/5 hover:border-white/10 transition-all flex items-center justify-between">
                  ...
                  <div className="text-right">
                     <div className="text-sm font-mono text-white">{node.metrics.cpu}%</div>
      ...
      <Card>
         <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-white">System Log Stream</h3>
            <span className="text-xs text-slate-500">Real-time Ingestion</span>
         </div>
         <div className="space-y-2">
            {logs.slice(0, 5).map(log => (
               <div key={log.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors border-b border-white/5 last:border-0">
                  <div className="flex items-center gap-4">
                     <div className={`w-8 h-8 rounded-full flex items-center justify-center border ${
                        log.level === 'error' ? 'border-red-500/30 bg-red-500/10' : 'border-slate-700 bg-slate-800'
                     }`}>
...
const DoraCryptoCard: React.FC<{
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl p-5 bg-[#0F1115] border border-white/5 hover:border-white/10 transition-all shadow-lg group">
      ...
      <div className="flex justify-between items-start mb-2 relative z-10">
         ...
         <div className="p-1 rounded-full bg-white/5 group-hover:bg-white/10 transition-colors cursor-pointer">
      ...
      <div className="mb-4 relative z-10">
         ...
         <div className={`text-xs font-medium mt-1 flex items-center gap-1 ${isGood ? 'text-emerald-400' : 'text-red-400'}`}>
            {isGood ? '▲' : '▼'} {Math.abs(trend)}% 
            <span className="text-slate-600 font-normal">vs last epoch</span>
         </div>
      </div>
