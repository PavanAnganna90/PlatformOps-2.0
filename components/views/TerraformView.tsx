...
import { Cloud, Play, CheckCircle, Terminal, AlertTriangle, Layers, ArrowRight, Loader2, RotateCcw, Lock, Unlock, X, PlusCircle, MinusCircle, FileDiff } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        <Card className="lg:col-span-8 flex flex-col bg-[#0d1117] border-slate-800 font-mono text-sm shadow-inner relative overflow-hidden">
          {/* Terminal Header */}
          <div className="flex items-center justify-between px-4 py-2 bg-slate-900 border-b border-slate-800">
             <div className="flex items-center gap-2 text-slate-400">
               <Terminal size={14} />
               <span className="text-xs">Console Output: {selectedWorkspace.name}</span>
               {selectedWorkspace.isLocked && (
             ...
             {/* Secondary Apply button in console header */}
             ...
             {selectedWorkspace.latestPlanOutput && (
               <>
                 ...
                 {planStats && selectedWorkspace.status === Status.PLANNED && (
                   <div className="mb-4 grid grid-cols-3 gap-4 p-4 bg-slate-900 rounded-lg border border-slate-800">
