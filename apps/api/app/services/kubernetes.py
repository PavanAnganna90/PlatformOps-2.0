...
class KubernetesService:
    ...
    def _load_kubeconfig(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            logger.info(
                f"Loaded kubeconfig from {kubeconfig_path}, context: {self._active_context}"
            )
            return True, None
    ...
    def _get_cluster_info(self, context: str, kubeconfig_path: Path) -> ClusterInfo:
        ...
        try:
            ...
            self._k8s_config.load_kube_config(config_file=str(kubeconfig_path), context=context)
    ...
    def get_pod_metrics(
        self, namespace: Optional[str] = None, cluster: Optional[str] = None
    ) -> Dict[str, dict]:
        """
